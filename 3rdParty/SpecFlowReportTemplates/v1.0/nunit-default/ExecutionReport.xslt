    ...
    <html>
      <head>
        ...
        <style>
          ...
          a.button.btn-toggle:before {content:"[";}
          a.button.btn-toggle:after {content:"]";}
          .hidden
          {
            display: none;
          }
    ...
    <tr>
      <td class="left">
        ...
        <xsl:if test="/sfr:NUnitExecutionReport/sfr:ScenarioOutput[@name = $testName]">
          <xsl:text> </xsl:text>
          <a href="#" onclick="toggle('out{$scenario-id}', event); return false;" class="button btn-toggle"><xsl:call-template name="get-common-tool-text">
      ...
      <td class="{$status}">
        ...
        <xsl:if test="$status = 'failure'">
          <xsl:text> </xsl:text>
          <a href="#" onclick="toggle('err{$scenario-id}', event); return false;" class="button btn-toggle"><xsl:call-template name="get-common-tool-text">
              <xsl:with-param name="text-key" select="'Show'" />
            </xsl:call-template></a>
        </xsl:if>
      </td>
